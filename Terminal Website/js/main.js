...
function enterKey(e) {
  ...
  if (pw) {
    ...
    if (pwd && e.keyCode == 13) {
      ...
      addLine("Password Correct. Type " + '<span class="command">admin</span>' +" to gain access to the website",  0);
      command.innerHTML = "";
      textarea.value = "";
      pw = false;
      liner.classList.remove("password");
    ...
    if (e.keyCode == 13) {
      ...
      git = commands.length;
      addLine("visitor@salahboussettah.online:~$ " + command.innerHTML, "no-animation", 0);
...
function commander(cmd) {
  switch (cmd.toLowerCase()) {
    ...
    case "password":
      ...
      addLine("<span class=\"inherit\"> Only one person knows that🤭 </span>", "error", 100);
      break;
    case "history":
      addLine("<br>", "", 0);
